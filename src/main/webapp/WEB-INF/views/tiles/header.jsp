          ...
          <li class="nav-item my-icon">
			...
			<c:if test = "${member == null }">
	            <a class="nav-link" aria-current="page" href="/member/login">
	            	<i class="header-icon bi bi-person-circle" style="font-size: 25px; padding-right: 10px;">Log In</i>
	            </a>
			</c:if>
          </li>
          <!--  로그인 한 상태 장바구니까지 보이게한다. -->
		  <c:if test="${ member != null }">
				      <a class="nav-link" aria-current="page" href="/member/logout.do">
	            	<span class="header-icon bi bi-person-circle" style="font-size: 25px; padding-right: 10px;">${member.user_name} Log Out</span>
	            </a>
          <li class="nav-item">
            <a class="nav-link" aria-current="page" href="/cart/${member.id}" >
				<i class="header-icon bi bi-minecart" style="font-size: 25px;">Cart</i>
			</a>
